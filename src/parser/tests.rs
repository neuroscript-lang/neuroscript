...
fn test_parse_use_nested_path() {
    ...
    assert_eq!(
        program.uses[0].path,
        vec!["blocks", "attention", "MultiHead"]
    );
}
...
"#;
    ...
    let neuron = &program.neurons["Test"];
    match &neuron.body {
        NeuronBody::Graph { connections, .. } => match &connections[0].destination {
        ...
        NeuronBody::Graph { connections, .. } => {
            match &connections[0].destination {
                Endpoint::Match(match_expr) => {
                    ...
                    assert!(
                        matches!(&match_expr.arms[1].pattern.dims[1], Dim::Named(s) if s == "d")
                    );
                }
                _ => panic!("Expected match endpoint"),
            }
...
fn test_parse_residual_example() {
    ...
    let NeuronBody::Graph {
        connections: residual_connections,
        ..
    } = &residual.body
    else {
    ...
    assert_eq!(
        residual_connections[0].source,
        Endpoint::Ref(PortRef::new("in"))
    );
    assert_eq!(
        residual_connections[0].destination,
        Endpoint::Call {
            name: "Fork".to_string(),
            args: vec![],
            kwargs: vec![],
    ...
    assert_eq!(
        residual_connections[1].destination,
        Endpoint::Tuple(vec![PortRef::new("main"), PortRef::new("skip")])
    );

    // 3. Ref(PortRef { node: "main", port: "default" }) -> Call { name: "MLP", args: [Name("dim")], kwargs: [] }
    assert_eq!(
        residual_connections[2].source,
        Endpoint::Ref(PortRef::new("main"))
    );
    assert_eq!(
        residual_connections[2].destination,
        Endpoint::Call {
            name: "MLP".to_string(),
    ...
    assert_eq!(
        residual_connections[3].destination,
        Endpoint::Ref(PortRef::new("processed"))
    );

    // 5. Tuple([PortRef { node: "processed", port: "default" }, PortRef { node: "skip", port: "default" }]) -> Call { name: "Add", args: [], kwargs: [] }
    assert_eq!(
        residual_connections[4].source,
        Endpoint::Tuple(vec![PortRef::new("processed"), PortRef::new("skip")])
    );
    assert_eq!(
        residual_connections[4].destination,
        Endpoint::Call {
            name: "Add".to_string(),
            args: vec![],
            kwargs: vec![],
    ...
    assert_eq!(
        residual_connections[5].destination,
